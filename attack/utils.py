...
def calculate_perturb(pass_clean, pass_reference, norm_perturb):
    ...
    cos_theta = torch.sum(pass_clean * pass_reference) / (
        norm_pass_clean * norm_pass_reference
    )
...
parser.add_argument(
    "--bd", dest="bd", action="store_true", help="let boundary detection work"
)
...
# np.random.seed(666)
clean_index = index[
    np.floor(np.linspace(0, index.shape[0] - 1, num_clean)).astype(np.int32)
]  # select clean examples
reference_index = np.setdiff1d(np.arange(50000), clean_index)  # remove clean examples
reference_index = np.random.choice(
    reference_index, num_reference, replace=False
).astype(
    np.int32
...
for i in range(num_clean):
    ...
    for j in range(num_reference):
        ...
        pass_reference_j = np.fft.ifft2(
            np.fft.ifftshift(fpass_reference_j)
    ...
    # attack succeeds
        ...
        index_adv = torch.nonzero(preds != label[i])[0][
            0
        ].item()  # first successful adversarial example
        if args.bl:
            ...
            if detection_query < 0:
                ...
                write_log(
                    log_file,
                    [i, index_adv + 1, torch.norm(adv_img[index_adv] - img).item()],
                )
            else:
