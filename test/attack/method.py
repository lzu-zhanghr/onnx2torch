...
class RayS:
    ...
    def get_xadv(self, x, v, d, lb=0.0, ub=1.0):
        if isinstance(d, int):
    ...
    def attack_hard_label(self, x, y, target=None, query_limit=10000, seed=None):
        ...
        for i in range(query_limit):
            ...
            attempt[valid_mask.nonzero().flatten(), start:end] *= -1.0
    ...
    def binary_search(self, x, y, target, sgn, valid_mask, tol=1e-3):
        ...
        initial_succ_mask = self.search_succ(
            self.get_xadv(x, sgn_unit, self.d_t), y, target, valid_mask
        )
        ...
        while len(to_search_ind) > 0:
            ...
            search_succ_mask = self.search_succ(
                self.get_xadv(x, sgn_unit, d_mid), y, target, to_search_ind
            )
            d_end[to_search_ind[search_succ_mask]] = d_mid[
                to_search_ind[search_succ_mask]
            ]
            d_start[to_search_ind[~search_succ_mask]] = d_mid[
                to_search_ind[~search_succ_mask]
            ]
        ...
        if len(to_update_ind) > 0:
            ...
            self.x_final[to_update_ind] = self.get_xadv(x, sgn_unit, d_end)[
                to_update_ind
            ]
